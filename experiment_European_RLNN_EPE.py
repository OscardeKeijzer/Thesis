...
for tenor_structure in tenor_structures:
    ...
    exp_dir_name = f'European Swaption EPE Profile {tenor_notation} Moneyness'
    ...
    time_t = 0.
    eval_times = np.arange(0., tenor_structure[0], 1/26)
    ...
    EPE_Bachelier_array = np.zeros_like(EPE_RLNN_array)
    ...
    for sim_count in range(n_sims):
        ...
        for moneyness in moneynesses:
            ...
            # Replicate swaption
            EuropeanRLNN = EuropeanSwaptionRLNN(fixed_rate, one_factor_HW_model, 
                                                notional, moneyness, 
            ...
            EuropeanRLNN.eval_EPE_profile(compare=True, time_t=time_t, eval_times=eval_times)
            ...
            # Store EPE profiles for computing means
            for m_count, m in enumerate(moneynesses):
                if moneyness == m:
                    moneyness_idx = m_count
                
            EPE_RLNN_array[moneyness_idx,sim_count] = EuropeanRLNN.EPE_RLNN
            EPE_Bachelier_array[moneyness_idx,sim_count] = EuropeanRLNN.EPE_Bachelier
            
            if sim_count > 0:
                plotRLNN =  EuropeanSwaptionRLNN(fixed_rate, one_factor_HW_model, 
                                                 notional, moneyness, swaption_type, 
                                                 tenor_structure)
                ...
                plotRLNN.EPE_Bachelier = np.concatenate(EPE_Bachelier_array[moneyness_idx,:sim_count+1], axis=-1)
                plotRLNN.plot_EPE_profile('compare', True, units_basis_points=True)
                plotRLNN.write_EPE_profile(True, True, True)
    
#%%
print(np.shape(EuropeanRLNN.x_train_idxs))
print(np.shape(EPE_RLNN_array))
#%% 
## SHUTDOWN COMMAND!! ##
