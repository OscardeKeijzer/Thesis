...
tenor_structures = [np.arange(5., 15.+1)] #[np.arange(1., 6.+1), np.arange(5., 10.+1), np.arange(5., 15.+1)]

for tenor_structure in tenor_structures:
    tenor_notation = f'NEW {int(tenor_structure[0])}Yx{int(tenor_structure[-1]-tenor_structure[0])}Y'
    exp_dir_name = f'Bermudan Swaption RLNN Sensitivities {tenor_notation}'
    ...
    for sim_count in range(n_sims):
        ...
        dVdR_RLNN_array[sim_count] = BermudanRLNN.dVdR
